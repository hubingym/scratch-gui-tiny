...
class SpriteSelectorItem extends React.PureComponent {
  ...
  render() {
    ...
    // 该组件被多个地方使用,目前在这儿该写最合适
    props.dragging = state.assetDrag.dragging;
    props.receivedBlocks = state.hoveredTarget.receivedBlocks && state.hoveredTarget.sprite === id;
...
SpriteSelectorItem.defaultProps = {
  ...
  onDrag(data) {
    state.assetDrag.updateAssetDrag(data);
  }
};
