...
import TargetPaneComponent from './target-pane/target-pane.jsx';
...
class TargetPane extends React.Component {
  ...
  handleChangeSpriteName(name) {
    const editingTarget = state.targets.editingTarget;
    this.props.vm.renameSprite(editingTarget, name);
  }
  ...
  handlePaintSpriteClick() {
    ...
    this.props.vm.addSprite(JSON.stringify(emptyItem)).then(() => {
      setTimeout(() => { // Wait for targets update to propagate before tab switching
        this.props.onActivateTab(state.editorTab.COSTUMES_TAB_INDEX);
      });
    });
  }
  handleActivateBlocksTab() {
    this.props.onActivateTab(state.editorTab.BLOCKS_TAB_INDEX);
  }
  ...
  handleBlockDragEnd(blocks) {
    const editingTarget = state.targets.editingTarget;
    if (this.props.hoveredTarget.sprite && this.props.hoveredTarget.sprite !== editingTarget) {
      this.props.vm.shareBlocksToTarget(blocks, this.props.hoveredTarget.sprite, editingTarget);
  ...
  render() {
    ...
    componentProps.editingTarget = state.targets.editingTarget;
    componentProps.sprites = state.targets.sprites;
    componentProps.stage = state.targets.stage;
    componentProps.raiseSprites = state.blockDrag.areBlocksOverGui;
    return (
      <TargetPaneComponent
        {...componentProps}
...
TargetPane.defaultProps = {
  ...
  onNewSpriteClick(e) {
    e.preventDefault();
    state.modals.openSpriteLibrary();
  },
  onRequestCloseSpriteLibrary() {
    state.modals.closeSpriteLibrary();
  },
  onReceivedBlocks(receivedBlocks) {
    state.hoveredTarget.setReceivedBlocks(receivedBlocks);
  },
  dispatchUpdateRestore(restoreState) {
    state.restoreDeletion.setRestore(restoreState);
  },
  onHighlightTarget(id) {
    state.targets.highlightTarget(id);
  },
  onCloseImporting() {
